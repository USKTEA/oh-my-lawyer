...
class ConstitutionalParser : LawApiParser {
    override fun parseTotalCount(searchResult: JsonNode): Int = searchResult.path("DetcSearch").path("totalCnt").asInt(0)
    ...
    override fun parseItemId(item: JsonNode): String? = item.textOrNull("헌재결정례일련번호")
        ...
        if (holding.isNotBlank() || summary.isNotBlank()) {
            ...
            chunks.add(
                ParsedChunk(
                    content,
                    ChunkType.SUMMARY,
                    mapOf("caseNumber" to caseNumber).toJsonString(),
                    chunkIndex++,
                ),
            )
        }
            ...
            for (section in TextChunker.chunkWithOverlap(fullText)) {
                chunks.add(
                    ParsedChunk(
                        section,
                        ChunkType.HOLDING,
                        mapOf("caseNumber" to caseNumber, "sectionIndex" to chunkIndex.toString()).toJsonString(),
                        chunkIndex++,
                    ),
                )
            }
