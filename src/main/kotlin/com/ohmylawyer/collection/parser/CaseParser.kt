...
class CaseParser : LawApiParser {
    override fun parseTotalCount(searchResult: JsonNode): Int = searchResult.path("PrecSearch").path("totalCnt").asInt(0)
    ...
    override fun parseItemId(item: JsonNode): String? = item.textOrNull("판례일련번호")
        ...
        if (holding.isNotBlank() || summary.isNotBlank()) {
            ...
            chunks.add(
                ParsedChunk(
                    content,
                    ChunkType.SUMMARY,
                    mapOf("caseNumber" to caseNumber, "courtName" to courtName).toJsonString(),
                    chunkIndex++,
                ),
            )
        }
            ...
            for (section in splitCaseText(fullText)) {
                chunks.add(
                    ParsedChunk(
                        section,
                        ChunkType.HOLDING,
                        mapOf("caseNumber" to caseNumber, "courtName" to courtName, "sectionIndex" to chunkIndex.toString()).toJsonString(),
                        chunkIndex++,
                    ),
                )
            }
