...
class CollectionController(
    ...
    ): ResponseEntity<CollectionCommandResponse> = ResponseEntity.accepted().body(collectionService.enqueue(resolveDataType(dataType)))

    @PostMapping("/all")
    fun collectAll(): ResponseEntity<List<CollectionCommandResponse>> = ResponseEntity.accepted().body(collectionService.enqueueAll())

    @GetMapping("/status")
    fun getStatus(): ResponseEntity<List<CollectionStatusResponse>> = ResponseEntity.ok(collectionService.getStatus())

    @PostMapping("/reset/{dataType}")
    fun resetProgress(
        @PathVariable dataType: String,
    ): ResponseEntity<CollectionCommandResponse> =
        if (dataType == "all") {
